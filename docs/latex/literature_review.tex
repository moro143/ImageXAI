...
Jest to jedna z najpopularniejszych wariantów ResNet używaną często podczas porównywania wyników, jako punkt odniesienia.
...
Istnieje wiele sposobu grupowania metod XAI.
Jednym z nich to podział na wyjaśnienia lokalne\cite{ribeiro2016why} i wyjaśnienia globalne\cite{XAIglobal}.
Wyjaśnienia globalne przedstawia cały model.
Natomiast wyjaśnienia lokalne skupiają się na wyjaśnianiu decyzji dla poszczególnych instancji danych.

Innym sposobem grupowania jest podział na dwie głózne grupy:
\begin{itemize}
...
\subsubsection*{LIME}
\textbf{LIME} \cite{ribeiro2016why, LIMEwhy} (Local Interpretable Model-agnostic Explanations) to jedna z najpopularniejszych technik XAI, która umożliwia tworzenie lokalnie interpretowalnych modeli wokół wybranych przypadków danych.
...
\subsubsection*{SHAP}
\textbf{SHAP} \cite{lundberg2017unified, SHAPmanip} (SHapley Additive exPlanations) wykorzystuje teorię gier do przypisywania znaczenia poszczególnym cechom wejściowym w procesie decyzyjnym modelu.
