...
Źródłami  problemów mogą być zbiory danych, w których wszystkie dane pochodzą z tego samego lub podobnego źródła, może to powodować przuczenie się modelu, poprzez nauczenie się modelu na nieoczekiwanych cech w celu klasyfikacji.
...
\begin{listing}
	\begin{minted}{python}
def explain_shap(instance)
    shap_values = initialize_shap_values(instance)
    for each feature j in instance:
        for each subset S excluding j:
            S_union_j = S U {j}
            pred_with_j = model.predict(instance with features in S_union_j)
            pred_without_j = model.predict(instance with features in S)
            contribution = pred_with_j - pred_without_j
            weight = (|S|! * (M - |S| - 1)!) / M!
            shap_values[j] += weight * contribution
    return shap_values
\end{minted}
